...
```
По умолчанию обучение идёт на CPU. Чтобы использовать GPU, укажите номер устройства:
```bash
python scripts/train_yolo.py --data data/rust_players.yaml --epochs 100 --img 960 --device 0
```
Если видите CUDA-ошибки, оставьте CPU:
```bash
